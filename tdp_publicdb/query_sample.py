...
def create_sample(views, sample, gene, data):
  ...
  co_expression = DBViewBuilder().idtype(gene.idtype).query("""
     SELECT d.{g.id} AS id, g.symbol, s.{s.id} as samplename, d.{{attribute}} AS expression, s.{{color}} as color
  ...
  expression_vs_copynumber = DBViewBuilder().idtype(gene.idtype).query("""
   SELECT d.{g.id} AS id, g.symbol, s.{s.id} as samplename, d.{{expression_subtype}} AS expression, d2.{{copynumber_subtype}} AS cn, s.{{color}} as color
       FROM {d.schema}.tdp_expression AS d
