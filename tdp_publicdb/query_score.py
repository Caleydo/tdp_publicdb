...
def create_gene_sample_score(result, gene, sample, data):
  ...
  result[basename + '_single_score'] = DBViewBuilder().idtype(gene.idtype).query("""
  ...
  result[basename + '_frequency_mutation_score'] = aggregate(
    """SUM({attribute}::integer) as count, COUNT({attribute}) as total""") \
    .replace('attribute', data.attributes) \
    .build()

  result[basename + '_frequency_score'] = aggregate(
  ...
  result[basename + '_frequency_copynumberclass_score'] = aggregate(
  ...
  result[basename + '_score'] = aggregate("""{agg_score} AS score""") \
    .replace('agg_score') \
    .build()
