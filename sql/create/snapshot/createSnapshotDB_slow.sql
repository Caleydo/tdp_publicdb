...
IMPORT FOREIGN SCHEMA public FROM SERVER origServer INTO _public;
IMPORT FOREIGN SCHEMA tissue FROM SERVER origServer INTO _tissue;
IMPORT FOREIGN SCHEMA cellline FROM SERVER origServer INTO _cellline;
--
CREATE TABLE public.targid_gene AS Select * from _public.targid_gene LIMIT :GENE_LIMIT;
ALTER TABLE public.targid_gene ADD CONSTRAINT pk_gene PRIMARY KEY(ensg);
CREATE INDEX ON public.targid_gene(symbol);
CREATE TABLE public.targid_geneassignment AS Select * from _public.targid_geneassignment where ensg in (select ensg from public.targid_gene);
...
CREATE TABLE tissue.targid_tissue AS Select * from _tissue.targid_tissue LIMIT :TISSUE_LIMIT;
...
CREATE TABLE tissue.targid_panelassignment AS Select * from _tissue.targid_panelassignment where tissuename in (select tissuename from tissue.targid_tissue);
ALTER TABLE tissue.targid_panelassignment ADD CONSTRAINT pk_tissueassignment PRIMARY KEY(panel, tissuename);
CREATE TABLE tissue.targid_mutation AS Select * from _tissue.targid_mutation where tissuename in (select tissuename from tissue.targid_tissue) and ensg in (select ensg from public.targid_gene) ;
...
CREATE TABLE tissue.targid_expression AS Select * from _tissue.targid_expression where tissuename in (select tissuename from tissue.targid_tissue) and ensg in (select ensg from public.targid_gene) ;
...
CREATE TABLE tissue.targid_copynumber AS Select * from _tissue.targid_copynumber where tissuename in (select tissuename from tissue.targid_tissue) and ensg in (select ensg from public.targid_gene) ;
...
CREATE TABLE cellline.targid_cellline AS Select * from _cellline.targid_cellline LIMIT :CELLLINE_LIMIT;
...
CREATE TABLE cellline.targid_panelassignment AS Select * from _cellline.targid_panelassignment where celllinename in (select celllinename from cellline.targid_cellline) ;
ALTER TABLE cellline.targid_panelassignment ADD CONSTRAINT pk_celllineassignment PRIMARY KEY(panel, celllinename);
CREATE TABLE cellline.targid_mutation AS Select * from _cellline.targid_mutation where celllinename in (select celllinename from cellline.targid_cellline) and ensg in (select ensg from public.targid_gene);
...
CREATE TABLE cellline.targid_expression AS Select * from _cellline.targid_expression where celllinename in (select celllinename from cellline.targid_cellline) and ensg in (select ensg from public.targid_gene);
...
CREATE TABLE cellline.targid_copynumber AS Select * from _cellline.targid_copynumber where celllinename in (select celllinename from cellline.targid_cellline) and ensg in (select ensg from public.targid_gene);
