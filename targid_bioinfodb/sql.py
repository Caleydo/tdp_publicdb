...
views = dict(
  ...
  gene_panel=DBViewBuilder().query("""
    SELECT genesetname AS id, species AS description FROM public.targid_geneset ORDER BY genesetname ASC""").build(),
  ...
  enrichment=DBViewBuilder().idtype(idtype_gene).query("""
    ...
    .replace('schema').replace('table_name').replace('entity_name').build(),
  ...
  onco_print=DBViewBuilder().idtype(idtype_tissue).query("""
    ...
    .arg("tumortype").arg("species").build(),
  ...
  onco_print_all=DBViewBuilder().idtype(idtype_tissue).query("""
    ...
    .arg("species")
    .replace("schema").replace("table_name").replace("entity_name").replace("ensgs").build(),
  ...
  row=DBViewBuilder().idtype(idtype_tissue).query("""
    ...
    .replace("schema").replace("table_name").replace("entity_name").replace("entities").build(),
  ...
  raw_data_table=DBViewBuilder().idtype(idtype_tissue).query("""
       ...
       WHERE b.tumortype = :tumortype AND b.species = :species""").replace("schema").replace("table_name").replace(
    "entity_name")
    .replace("data_subtype")
    .arg("tumortype").arg("species").build(),
  ...
  co_expression=DBViewBuilder().idtype(idtype_gene).query("""
    ...
    .replace("schema").replace("entity_name").replace("expression_subtype").build(),
  ...
  expression_vs_copynumber=DBViewBuilder().idtype(idtype_gene).query("""
     ...
     WHERE C.tumortype = :tumortype AND a.ensg = :ensg""")
    .arg("ensg").arg("tumortype")
    .replace("schema").replace("entity_name").replace("expression_subtype").replace("copynumber_subtype").build(),
  ...
  frequency_score=DBViewBuilder().idtype(idtype_gene).query("""
    ...
    .replace("schema").replace("entity_name").replace("table_name").replace("data_subtype").replace("operator").replace("value").build(),
  ...
  mutation_frequency=DBViewBuilder().idtype(idtype_gene).query("""
       ...
       ON mut.ensg = a.ensg""")
    .arg("tumortype").arg("species")
    .replace("schema").replace("entity_name").replace("data_subtype").build(),
